...
- `mini_program_open`：打开小程序，小程序进入前台时触发，包括首次打开和从后台切回，携带 `user_id`、`scene`
- `mini_program_hide`：小程序进入后台，用户关闭小程序、切到后台或切到其他 App 时触发，携带 `user_id`、`mini_program_duration_ms`
- `home_tab_click`：点击首页 tab，点击底部导航“首页”tab 时触发，携带 `user_id`
...
- `record_edit_page_view`：记录编辑页访问，用于统计记录编辑页 PV 和 UV，进入记录编辑页时触发，携带 `user_id`、`record_id`、`record_type`
- `record_edit_success`：记录编辑保存成功，后端更新记录成功后触发，携带 `user_id`、`record_id`、`record_type`
- `create_tab_click`：点击新建页 tab，点击底部导航“新建”tab 时触发，携带 `user_id`
...
- `stats_tab_click`：点击统计页 tab，点击底部导航“统计”tab 时触发，携带 `user_id`
...
- `create_page_view`：新建类型入口页访问，用于统计新建类型入口页 PV 和 UV，用户进入新建类型入口页时触发，携带 `user_id`
- `contacts_tab_click`：点击联系人页 tab，点击底部导航“联系人”tab 时触发，携带 `user_id`
...
- `mine_tab_click`：点击我的页 tab，点击底部导航“我的”tab 时触发，携带 `user_id`
...
- `data_clear_click`：点击清空数据，点击数据管理页清空数据入口时触发，携带 `user_id`
- `data_clear_success`：清空数据成功，点击确认按钮且后端返回成功时触发，携带 `user_id`、`record_count`
...
- `feedback_submit_success`：意见反馈成功，点击提交反馈且后端返回成功时触发，携带 `user_id`、`feedback_type`
